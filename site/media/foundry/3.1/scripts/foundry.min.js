...
$.isPlainObject(args[0])?(options=args[0],names=args.slice(1)):names=args,options=$.extend({},self.defaultOptions,batch.options,{path:$.path+"styles/default.htm"},options,{batch:batch}),$.each(names,function(i,name){var task=new self.task(name,options),existingTask=self.stylesheets[task.url];task=existingTask||task,batch.addTask(task),existingTask||(self.stylesheets[task.url]=task,task.start())})};return $.extend(self,{defaultOptions:{path:"",extension:"compressed"==$.mode?"min.css":"css",xhr:!1},setup:function(){$.extend(self.defaultOptions,options)
